<?php
    ...
    if ( ($_SERVER['REQUEST_METHOD'] == 'POST') and isset($_POST['login']) )
    {
        ...
        if ($login_ok){
            ...
            // 여기서 방문자 처리
            $_SESSION['visit'] = "1";
            $stmt = $con->prepare('insert into tb_stat_visit (regdate, regip, referer) values(:currdt, :userip, :referer)');
    ...
    // 오늘 방문자수
    try{
        $stmt = $con->prepare('select count(*) as count from tb_stat_visit where DATE(regdate) = DATE(:currdt)');
        $stmt->bindParam(':currdt', $currdt);
        $stmt->execute();
    ...
    try{
        // 전체 방문자수
        $stmt = $con->prepare('select count(*) as count from tb_stat_visit');
        $stmt->execute();
                ...
                <div class="box__in__mid">
                        ...
                        <div class="box__in__mid_margin">
                            <div class="box__in__mid_input">
                                <form method="POST">
                                    <p>
                                        <input class="w3-input w3-padding-14" type="text" placeholder="사번" required name="user_name" maxlength="30">
                                    </p>
                                    ...
                                    <p>
                                        ...
                                        <input class="w3-input w3-padding-14" type="password" placeholder="이름" required name="user_password" maxlength="30">
                                    </p>
