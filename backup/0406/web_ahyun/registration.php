	...
	//		$errMSG = "잘못된 패스워드";
		...
		if(empty($username)){
			...
			$errMSG = "사번을 입력하세요.";
                ...
                try { 
                    $stmt = $con->prepare('select * from '.$usersdb.' where username=:username');
		...
		{
                   try{
			$stmt = $con->prepare('INSERT INTO '.$usersdb.'(username, usernumber, password, userprofile, salt) VALUES(:username, :usernumber, :password, :userprofile, :salt)');
			$stmt->bindParam(':username',$username);
...
<form id="form" method="post" enctype="multipart/form-data" class="form-horizontal" style="margin: 0 300px 0 300px;border: solid 1px;border-radius:4px">
    ...
    <tr>
        <? $r5 = md5(mt_rand()); ?>
    	<td><label class="control-label">사번</label></td>
        <td><input class="form-control" type="text" name="<? echo $r5; ?>" placeholder="사번을 입력하세요" autocomplete="off" readonly 
    onfocus="this.removeAttribute('readonly');" />
            <input type="hidden" name="__autocomplete_fix_<? echo $r5; ?>" value="newusernumber" /> 
        </td>
    </tr>
    <tr>
